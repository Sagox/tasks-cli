...
module.exports = {
  ...
  addTasklist: async (tasklists, service) => {
    return new Promise(async (resolve, reject) => {
      var newTasklist = await inquirer.tasklistForm();
      var tasklist_titles = tasklists.map(tasklist => tasklist.title);

      if(tasklist_titles.includes(newTasklist.title))
        ;
      else {
        await tasksApi.createNewTaskList(newTasklist.title, service);
      }
      resolve();
    });
  },
  ...
  deleteTasklist: async (tasklists, service) => {
    return new Promise(async (resolve, reject) => {
      var tasklist = await inquirer.getTasklist(tasklists);
      var tasklist_titles = tasklists.map(tasklist => tasklist.title);
      var tasklistID = tasklists[tasklist_titles.indexOf(tasklist.option)].id;
      await tasksApi.deleteTasklist(tasklistID, service);
      resolve();
    });
  },
  fresh: async() => {
